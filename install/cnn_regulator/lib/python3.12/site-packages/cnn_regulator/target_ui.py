...
class TargetControlUI(Node):
    def __init__(self):
        ...
        # Publisher for target position updates to the CNN controller
    ...
    def publish_target(self, target):
        """Publish a target position to the CNN controller."""
        target = np.asarray(target, dtype=float)

        if target.shape != (6,):
            print(f"Error: Expected 6 target values, got {target.shape}")
            return False
    ...
    def set_target(self, target):
        """Set target position and send it to the CNN controller."""
        self.target_position = target.copy()
        if self.publish_target(target):
            self.get_logger().info(f"✓ Target sent to CNN controller: {np.round(target, 3)}")
            return True

        print("\n⚠ Failed to publish target.")
        print("Make sure CNN controller is running: ros2 run cnn_regulator ros_controller")
        return False
...
def main(args=None):
    ...
    ui.publish_target(ui.target_position)
